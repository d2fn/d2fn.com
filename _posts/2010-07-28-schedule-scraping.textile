...
Bingo.  We see a file sessions.json and speakers.json which, upon closer inspection contain all the information we need about the session schedule.  Some ugly command line magic (omitted here) gives us a flat file with each session guid on its own line. 
